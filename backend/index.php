...
// Handle PUT request for the first API
if ($_SERVER['REQUEST_METHOD'] === 'PUT' && isset($_GET['api']) && $_GET['api'] === 'first') {
    // Retrieve JSON data from the request body
    $rawData = file_get_contents('php://input');
    ...
    if (json_last_error() === JSON_ERROR_NONE) {
        ...
        curl_setopt_array($ch, [
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_CUSTOMREQUEST => 'PUT', // Explicitly set the request method to PUT
            CURLOPT_POSTFIELDS => $postData,
            CURLOPT_HTTPHEADER => [
                'Content-Type: application/json',
            ],
        ...
        // JSON decoding failed, log the error
        $jsonError = json_last_error_msg();
        file_put_contents('debug_log.txt', "JSON Decode Error: " . $jsonError . PHP_EOL, FILE_APPEND);

        http_response_code(400); // Bad Request
        echo json_encode(["error" => "Error decoding JSON data", "details" => $jsonError]);
    }
    exit;
}
