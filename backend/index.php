...
// Handle POST request for the second API (REVIEW SECTION)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_GET['api']) && $_GET['api'] === 'second') {
    ...
    // Extract 'name' and 'review' fields from the data
    $extractedData = [
        'name' => isset($data['name']) ? $data['name'] : '',
        'review' => isset($data['review']) ? $data['review'] : ''
    ];

    // Prepare extracted data to be sent as JSON
    $postData = json_encode($extractedData);

    // Check if JSON decoding was successful
    if (json_last_error() === JSON_ERROR_NONE) {
        // URL of the API endpoint for the microservice
        $apiUrl = 'https://manjeshprasad.com/DBMS/Review.php';
        ...
        if ($response === false) {
            ...
            error_log("cURL response: " . $response); // Log cURL response
        ...
        error_log("JSON decode error: " . $error); // Log JSON error
        echo json_encode(["error" => "Error decoding JSON data: " . $error]);
    }
    exit;
}
